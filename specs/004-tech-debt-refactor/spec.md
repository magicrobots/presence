...
- **FR-002a**: `any` is strictly and unconditionally forbidden per the project constitution (§4) — no exceptions, no documentation procedures, no PR-level carve-outs. Where a genuinely un-typeable dynamic pattern cannot be resolved, the developer MUST use `unknown` with a narrowing type guard. The narrowing guard approach MUST be documented with an inline comment explaining why the type cannot be narrowed statically. If even `unknown` with narrowing cannot express the pattern without disproportionate effort, the developer MUST consult the tech lead before proceeding — `any` is never the answer.
...
- Q: Constitution §3 defines the API response envelope as `{ success: boolean, data: any, error?: string }`, using `any`. Does this conflict with FR-002a's unconditional prohibition on `any`, and does this feature need to resolve it? → A: **No conflict in scope — the API envelope `data: any` in constitution §3 is a pre-existing limitation that is explicitly out of scope for this feature.** This feature's `any` prohibition (FR-002a, constitution §4) applies to: all `frontend/src/` code, all new TypeScript written in `packages/types/` if gaps are found during the TS migration. It does NOT apply to the `/api` workspace, which is entirely out of scope per the Assumptions section. The constitution's self-contradiction (§3 uses `any` while §4 prohibits it) is an acknowledged pre-existing inconsistency; resolving it would require a separate constitution update outside this feature. Implementers on this feature branch MUST NOT use `any` in any in-scope file, and MUST NOT interpret the API envelope's `data: any` as permission to use `any` anywhere in frontend or types code. The API envelope type is not touched by this feature.

- Q: Constitution §6 ("TS requirement override") states JS/JSX is permitted during the initial migration phase. Does this feature formally close that override? → A: **Yes — §6 is closed by this feature.** Constitution §6 was written to allow JS/JSX "during initial update phase (responsive-crt-display phase)." That phase is complete. This feature (`004-tech-debt-refactor`) IS the TypeScript migration phase referenced in §6. Upon merge of this feature, §6's condition is fully met: all frontend source files are TypeScript strict mode. No JS/JSX source files will remain in `frontend/src/`. After this feature ships, §6 no longer provides any exemption — strict TypeScript is the law of the codebase per constitution §2 and §4. **Implementers MUST treat this clarification as superseding constitution §6 for all work on this feature branch. No `.js` or `.jsx` source files are permitted in `frontend/src/` under any reading of §6 — that override is fully satisfied and expired as of this feature. The constitution cannot be modified from spec artifacts (guardrail 998); this spec-layer clarification is the normative record and takes precedence over the stale §6 text in constitution.md for this feature.**
...
- Q: If a specific area genuinely cannot be strictly typed without disproportionate effort, is a documented `any`/`unknown` acceptable as a last resort? → A: `unknown` with narrowing only — `any` is strictly and unconditionally forbidden per the project constitution (§4) with zero exceptions. Where strict typing genuinely cannot be achieved without disproportionate effort, `unknown` with a type-narrowing guard is the only permitted alternative; the reason MUST be documented with an inline comment. There is no procedure that makes `any` acceptable. See FR-002a.
