...
- **FR-005**: The CRT canvas rendering MUST continuously monitor frame performance during the animation loop and dynamically adjust effect quality to maintain smooth rendering on the host device. When sustained frame time exceeds the 60fps budget (>16.7ms per frame), the system MUST reduce effect fidelity (e.g., simplified scanlines, reduced glow passes) until rendering is smooth. When performance headroom is detected after a reduction, the system MAY attempt to restore quality. Quality adjustment MUST be seamless — no visible flash or layout shift.
- **FR-006**: The display MUST correctly maintain the 4:3 aspect ratio and effect quality when the window is resized.
- **FR-007**: The continuous performance monitor MUST re-evaluate and reset quality state whenever the canvas area increases beyond the size at which the current quality level was established. If the new canvas area (width × height in pixels) exceeds the area recorded at last quality calibration, the monitor MUST restart its evaluation at the new size. Decreasing the window size MUST NOT reset quality state — the display remains at its current quality level.
...
- "Smooth" rendering is device-agnostic: the application continuously monitors its own frame performance and adjusts CRT effect quality to sustain ≤16.7ms per frame (60fps target) on whatever hardware is present. No minimum hardware baseline is required — the quality adaptation mechanism is the guarantee.
...
- **SC-002**: Rendering achieves and maintains visual smoothness at any display size through continuous quality adaptation — after an initial settling period (≤5 seconds), frame time stabilizes at ≤16.7ms per frame (60fps target) and holds there for ≥10 seconds of continuous rendering, with no more than 2 dropped frames per second. Verified via browser DevTools Performance panel or `requestAnimationFrame` timestamp deltas. The quality level at which smoothness is achieved may vary by device; that is acceptable.
