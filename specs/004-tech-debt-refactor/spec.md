...
- Q: If a specific area genuinely cannot be strictly typed without disproportionate effort, is a documented `any`/`unknown` acceptable as a last resort? → A: `unknown` with narrowing only — `any` remains strictly forbidden per the project constitution (§4) with no exceptions. Where strict typing genuinely cannot be achieved, `unknown` with a type-narrowing guard is the required alternative; the specific reason MUST be documented with an inline comment and the occurrence tracked in the PR description. See FR-002a for the full procedure.
